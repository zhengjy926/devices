...
//#ifdef ADI_DEBUG
    ...
    {
        LOG_E("CHIPID read error:0x%04x. AD5940 is not present?\n", i);
        return;
    }
#ifdef CHIPSEL_M355
    ...
    LOG_D("This AD594x!\n");
#endif
    LOG_D("Note: Current Silicon is %s\n", bIsS2silicon?"S2":"S1");
    LOG_D("AD5940LIB Version:v%d.%d.%d\n", AD5940LIB_VER_MAJOR, AD5940LIB_VER_MINOR, AD5940LIB_VER_PATCH);
//#endif
}
...
{
    LL_GPIO_ResetOutputPin(BSP_SPI1_CS_PORT, BSP_SPI1_CS_PIN);
}
void AD5940_CsSet(void)
{
    LL_GPIO_SetOutputPin(BSP_SPI1_CS_PORT, BSP_SPI1_CS_PIN);
}
void AD5940_RstClr(void)
{
    gpio_write(AD5940_RST_PIN_ID, 0);
}
void AD5940_RstSet(void)
{
    gpio_write(AD5940_RST_PIN_ID, 1);
}
void AD5940_Delay10us(uint32_t time)
{
    bsp_dwt_delay_us(time*10);
}
...
void AD5940_interrupt_handle(void *args)
{
    ucInterrupted = 1;
}
...
{
    /* 初始化 RST 引脚 */
    gpio_set_mode(AD5940_RST_PIN_ID, PIN_OUTPUT_PP, PIN_PULL_UP);
    gpio_write(AD5940_RST_PIN_ID, 1);
    
    /* 初始化中断输入引脚 */
    gpio_set_mode(AD5940_GPIO0_INT, PIN_INPUT, PIN_PULL_UP);
    gpio_attach_irq (AD5940_GPIO0_INT, PIN_EVENT_FALLING_EDGE, AD5940_interrupt_handle, NULL);
    gpio_irq_enable (AD5940_GPIO0_INT, 1);
    
    return 0;
}

/**
* @brief  SPI 块传输函数 (轮询模式)
* @param  SPIx:     SPI 外设 (例如 SPI1)
* @param  pTxData:  发送缓冲区指针 (如果为 NULL，则发送 0xFF)
* @param  pRxData:  接收缓冲区指针 (如果为 NULL，则数据被丢弃)
* @param  Size:     传输数据的长度
* @param  Timeout:  超时时间 (ms)
* @retval 0: 成功, -1: 超时
*/
int8_t SPI_TransmitReceive_Buffer(SPI_TypeDef *SPIx, uint8_t *pTxData, uint8_t *pRxData, uint16_t Size, uint32_t Timeout)
{
    uint32_t start_tick;
    uint16_t i;
    uint8_t tx_val;
    ...
    for (i = 0; i < Size; i++)
    {
        ...
        {
                tx_val = 0xFF; // 发送 Dummy Byte (通常为 0xFF 或 0x00)
        }
        ...
        // --- 3. 发送数据 ---
        LL_SPI_TransmitData8(SPIx, tx_val);
        ...
        // --- 5. 读取接收数据 ---
        // 即使不需要接收数据，也必须读取 DR 寄存器以清除 RXNE 标志和防止溢出
        rx_val = LL_SPI_ReceiveData8(SPIx);
...
{
    SPI_TransmitReceive_Buffer(SPI1, pSendBuffer, pRecvBuff, length, 100);
}
